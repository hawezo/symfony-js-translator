...
class Translator {
		...
		for (_e in _explicitRules) {
			if (this.settings.iPluralRegex.test(_e)) {
				...
				if (_matches[1]) {
					...
					var _leftNumber  = this.convertNumber(_matches[4]);
					var _rightNumber = this.convertNumber(_matches[5]);
	...
	/**
     * Convert number as String, "Inf" and "-Inf" values to number values.
     * This code is sjamelessly stolen from @see https://github.com/willdurand/BazingaJsTranslationBundle/blob/master/Resources/js/translator.js#L427 - thank you, Will.
     * 
     * @param {String} number 
     */
